...
class Robot(Thread):
  ...
  def run(self):
    ins = {"n": "north", "s": "south", "e": "east", "w": "west"}
    initial = collections.deque(
      ["north", "take sand", "north", "take space heater", "east",
       "take semiconductor", "west", "south", "south", "east",
       "take ornament", "south", "take festive hat", "east",
       "take asterisk", "south", "west", "take food ration", "east",
       "east", "take cake", "west", "north", "west", "north", "west",
       "west", "north", "north", "inv"])

    stuff = ["asterisk", "cake", "festive hat", "food ration",
    ...
    while True:  #not self.stop_event.set():
      ...
      if s.startswith("Command"):
        ...
        if inp == "4":
          combos = list(itertools.combinations(stuff, 4))
          for combo in combos:
            for thing in stuff:
              initial.append("drop %s" % thing)
            print "Taking", combo
